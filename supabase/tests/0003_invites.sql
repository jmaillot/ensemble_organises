...
select
  'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'::text as h1,
  'bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb'::text as h2,
  'cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc'::text as h3,
  'dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd'::text as h4,
  'eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee'::text as h5,
  'ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff'::text as h6;
...
select testkit.expect_denied(format(
  ...
  (select user_id from fx where key = 'alice'), (select household_id from fx where key = 'alice'),
  (select h1 from hashes)));
select testkit.expect_denied(format(
...
select testkit.expect_denied(format(
  ...
  (select h1 from hashes), (select user_id from fx where key = 'alice'), 'Alice'));
...
select testkit.eq(
  (public.create_household_invite_token(
    p_actor_id => (select user_id from fx where key = 'alice'),
    p_household_id => (select household_id from fx where key = 'alice'),
    p_token_hash => (select h1 from hashes),
    p_max_uses => 10
  ) ->> 'max_uses')::int,
  10,
...
select testkit.eq(
  (select count(*) from public.household_invite_tokens
    where household_id = (select household_id from fx where key = 'alice')),
  1::bigint,
  'un token est créé pour le foyer');

select testkit.ok(
  (select token_hash from public.household_invite_tokens
    where household_id = (select household_id from fx where key = 'alice')) = (select h1 from hashes),
...
select testkit.expect_denied(format(
  'select public.create_household_invite_token(p_actor_id => %L, p_household_id => %L, p_token_hash => %L)',
  (select user_id from fx where key = 'bob'), (select household_id from fx where key = 'alice'),
  (select h2 from hashes)));

-- Un administrateur d'un autre foyer non plus.
select testkit.expect_denied(format(
  'select public.create_household_invite_token(p_actor_id => %L, p_household_id => %L, p_token_hash => %L)',
  (select user_id from fx where key = 'carol'), (select household_id from fx where key = 'alice'),
  (select h2 from hashes)));
...
select testkit.expect_denied(format(
  'select public.create_household_invite_token(p_actor_id => %L, p_household_id => %L, p_token_hash => %L)',
  (select user_id from fx where key = 'alice'), (select household_id from fx where key = 'alice'), 'pas-un-hmac'));
...
select public.create_household_invite_token(
  p_actor_id => (select user_id from fx where key = 'alice'),
  p_household_id => (select household_id from fx where key = 'alice'),
  p_token_hash => (select h2 from hashes),
  p_max_uses => 5
);
...
select testkit.eq(
  (select is_active from public.household_invite_tokens where token_hash = (select h1 from hashes)), false,
  'le token précédent est désactivé');
select testkit.expect_denied(format(
  'select public.redeem_household_invite_token(%L, %L, %L)',
  (select h1 from hashes), (select user_id from fx where key = 'dave'), 'Dave'),
...
select testkit.expect_denied(format(
  'select public.redeem_household_invite_token(%L, %L, %L)',
  (select h3 from hashes), (select user_id from fx where key = 'dave'), 'Dave'),
  'une empreinte inconnue est refusée');

select testkit.eq(
  (public.redeem_household_invite_token(
    p_token_hash => (select h2 from hashes),
    p_user_id => (select user_id from fx where key = 'dave'),
...
select testkit.eq(
  (select use_count from public.household_invite_tokens where token_hash = (select h2 from hashes)), 1,
...
select testkit.eq(
  (select role from public.household_members
    where user_id = (select user_id from fx where key = 'dave')), 'membre',
...
select testkit.eq(
  (select display_name from public.household_members
    where user_id = (select user_id from fx where key = 'dave')), 'Dave Durand',
  'le nom du membre provient du profil');
select testkit.eq(
  (select household_id from public.household_members
    where user_id = (select user_id from fx where key = 'dave')), (select household_id from fx where key = 'alice'),
...
select public.create_household_invite_token(
  p_actor_id => (select user_id from fx where key = 'alice'),
  p_household_id => (select household_id from fx where key = 'alice'),
  p_token_hash => (select h4 from hashes),
  p_max_uses => 1
);
...
select testkit.eq(
  (public.redeem_household_invite_token(
    p_token_hash => (select h4 from hashes),
    p_user_id => (select user_id from fx where key = 'erin'),
    p_display_name => 'Erin Petit'
  ) ->> 'already_member')::boolean,
  false,
  'le premier usage réussit');
select testkit.eq(
  (select use_count from public.household_invite_tokens where token_hash = (select h4 from hashes)), 1,
  'le compteur est incrémenté');
select testkit.eq(
  (select is_active from public.household_invite_tokens where token_hash = (select h4 from hashes)), false,
  'un token épuisé est désactivé');
select testkit.expect_denied(format(
  'select public.redeem_household_invite_token(%L, %L, %L)',
  (select h4 from hashes), (select user_id from fx where key = 'frank'), 'Frank'),
...
select public.create_household_invite_token(
  p_actor_id => (select user_id from fx where key = 'alice'),
  p_household_id => (select household_id from fx where key = 'alice'),
  p_token_hash => (select h5 from hashes),
...
update public.household_invite_tokens
 ...
 where token_hash = (select h5 from hashes);

select testkit.expect_denied(format(
  'select public.redeem_household_invite_token(%L, %L, %L)',
  (select h5 from hashes), (select user_id from fx where key = 'frank'), 'Frank'),
  'un token expiré est refusé');
select testkit.eq(
  (select is_active from public.household_invite_tokens where token_hash = (select h5 from hashes)), false,
...
select testkit.eq(
  (public.create_household_invite_token(
    p_actor_id => (select user_id from fx where key = 'alice'),
    p_household_id => (select household_id from fx where key = 'alice'),
    p_token_hash => (select h6 from hashes),
...
select testkit.eq(
  (select count(*) from public.household_invite_tokens
    where household_id = (select household_id from fx where key = 'alice') and is_active), 0::bigint,
...
update public.household_invite_tokens
 ...
 where token_hash = (select h6 from hashes);

select testkit.eq(private.prune_expired_invite_tokens() >= 1, true,
  'la purge désactive les tokens expirés');
select testkit.eq(
  (select is_active from public.household_invite_tokens where token_hash = (select h6 from hashes)), false,
